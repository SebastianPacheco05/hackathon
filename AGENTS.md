...
- Start here for cross-project norms. Revital is a single-project repository with only one main application (`revital_ecommerce`) its guidelines are placed in `AGENTS.md`.
...
**Scopes:** `ecommerce`, `ecommerce-backend`, `ecommerce-frontend`
...
- **Independence**: `revital_ecommerce` is completely independent
